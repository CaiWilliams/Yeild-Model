...
irradiance_dir = os.path.join(os.getcwd(),'Locations','Plymouth','Plymouth_Irradiance.csv')
weather_dir = os.path.join(os.getcwd(),'Locations','Plymouth','Plymouth_weather_clean.csv')
device_dir = os.path.join(os.getcwd(),'Devices','c-Si.csv')
device_dir2 = os.path.join(os.getcwd(),'Devices','CdTe.csv')
location = setup_loc(50.375445,-4.142583, irradiance_dir,weather_dir)
power = run(location,device_dir)
power2 = run(location,device_dir2)

print(np.sum(power)/1000)
print(np.sum(power2)/1000)
